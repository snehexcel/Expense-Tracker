...
def generate_pie_chart():
    ...
    all_data = connector.execute('SELECT ModeOfPayment, SUM(Amount) FROM ExpenseTracker GROUP BY ModeOfPayment')
    data = all_data.fetchall()
    ...
    plt.title('Expense Distribution by Mode of Payment')
    plt.show()
...
Button(buttons_frame, text='Generate Pie Chart', font=btn_font, width=25, bg=hlb_btn_bg,
       command=generate_pie_chart).place(x=640, y=65)
